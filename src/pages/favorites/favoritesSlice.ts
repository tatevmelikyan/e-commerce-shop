...
const favoritesSlice = createSlice({
  ...
  reducers: {
    ...
    updateLikedProducts: (state, action: {payload: IProduct, type: string}) => {
     const isProductLiked = state.favoriteItems.find((item: IProduct) => item.id === action.payload.id)
     if(isProductLiked) {
      state.favoriteItems = state.favoriteItems.filter((item: IProduct) => item.id !== action.payload.id)
